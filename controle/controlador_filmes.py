...
class ControladorFilmes:
    ...
    def dados_lista_filmes_por_genero(self):
        generos = [genero for genero in self.__controlador_sistema.controlador_generos.generos if
                   len(genero.filmes) > 0]
        filmes = [filme for filme in (genero.filmes for genero in generos)]
        filmes_por_genero = []
        for genero, filme_list in zip(generos, filmes):
            filmes_por_genero.append(f'{genero.tipo}:')
            for filme in filme_list:
                filmes_por_genero.append(f'  ID: {filme.id_filme}   {filme.titulo}')
    ...
    def lista_filmes_por_genero(self):
        if self.existem_filmes_cadastrados() and self.__controlador_sistema.controlador_generos.existem_generos_cadastrados():
            self.__tela_filme.popup_lista_filmes_por_genero(self.dados_lista_filmes_por_genero())
    ...
    def abre_tela(self):
        lista_opcoes = {
            ...
            5: self.lista_filmes_por_genero
        }
